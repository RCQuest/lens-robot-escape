...
long integral = 0;
int speed = 150;
int braking_factor_pos = (1+((power_difference*0)/maximum);
int braking_factor_neg = (1-((power_difference*0)/maximum);
...
//initial speed - allows the keys 'A' and 'C' to adjust the maximum speed
set_motors(0,0);                    // stop the motors
    wait_for_button_release(BUTTON_B);
    delay(200);                      // debounce button B
    ...
    { 
      ...
      if(button_is_pressed(BUTTON_C))
      {
        ...
        if( speed > 255 )
          speed = 255;
        delay(200);                  // debounce button C
      }
    }   
    wait_for_button_release(BUTTON_B);
	// end of initial speed
...
void loop()
{
  ...
  // improve performance.
  int power_difference = proportional/10 + integral/9000 + derivative*2;

  // Compute the actual motor settings.  We never set either motor
  // to a negative value.
  const int maximum = speed;
  if (power_difference > maximum)
    power_difference = maximum;
  if (power_difference < -maximum)
    power_difference = -maximum;

//speed of both wheels is reduced by a factor between zero and one which is proportional to the power_difference
  if (power_difference < 0)
    OrangutanMotors::setSpeeds((maximum + power_difference)*braking_factor_pos, maximum*braking_factor_pos);
  else
    OrangutanMotors::setSpeeds(maximum*braking_factor_neg, (maximum - power_difference)*braking_factor_neg);
  }
