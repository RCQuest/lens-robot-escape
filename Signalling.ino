...
//if the floor is white, enter loop. Some tolerance on surface fluctuations (only works with white paper).
  if (position < 50 || position > 3950)
  {
    //carry on straight
    OrangutanMotors::setSpeeds(maximum, maximum);
    //This delay allows Bwyan to traverse a 10cm white patch (speed independant).
    delay(100*255/maximum);

      //if he finds black again, enter loop.
      if (50 < position || position < 3950)
      {
         OrangutanBuzzer::playFromProgramSpace(welcome);
         //Increment signal counter, a signalling patch has been detected
         signal_counter++;
      }
  }

// after a set number of signalling strips do something... in this case slow down
 if (signal_counter == 3)
  {
        // this delay allows Bwyan some time to make it out of the final signalling white strip
        delay(350);
        // the 'do something' here is to slow down
        OrangutanMotors::setSpeeds(50, 50);
        // or use 'maximum =50'
        OrangutanBuzzer::playFromProgramSpace(go);
        
        //signalling ended, revert to line following.
  }
